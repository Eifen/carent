...
class HoursController extends Controller
{
    ...
    public function prepareAddHour(Request $registerRequest)
    {
        ...
        foreach ($arrayListHours as $cursor => $hourCharged) {
            if ($hourCharged["user_id"] == $userId && $hourCharged["register_date"] == $date && $hourCharged["user_assigned_id"] == $this->projectAssignedId) {
        ...
        $totalHours = ($arrayProjectInfoDTO[$positionProjectInfo]["hoursLoad"] - $getHourCharged) + $hourValue;
        ...
        $response = HoursModel::addHour($prepareHourInfo, $getLoadId);
