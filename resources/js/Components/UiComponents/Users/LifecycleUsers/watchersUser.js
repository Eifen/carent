    ...
    watch: {
        ...
        inputSelect(newSelect, oldSelect) {
            try {
                ...
                if (this.inputSelect.length > 2)
                    this.messages.error.documentError = "";
        ...
        inputCode(newCode, oldCode) {
            ...
            if (this.inputCode.length > 0 && this.inputCode.length <= 6)
                this.submitButton.codeValid = true;
        },
        //Activamos de Inputs Watchers
        inputEstadoSelect(getEstado) {
            //Consultamos los municipios
            this.municipality.select = this.municipality.init.filter(
                (municipality) => {
                    return municipality.state_id === getEstado;
                }
            );
        },
        inputMunicipioSelect(getMunicipio) {
            //Consultamos las parroquias
            this.parish.select = this.parish.init.filter((parish) => {
                return parish.municipality_id === getMunicipio;
            });
        },
    },
};
