...
export const createdMixin = (self) => {
    self.inputWatchers = 
    [{
        propiedades: ['inputStatusSelect', 'inputCurrenciesSelect', 'inputCompaniesSelect'],
        watch: (actualTarget) => 
        {
            //Control de banderas
            if(actualTarget >= 1){
                if(self.inputStatusSelect != 0) self.submitButton.statusValid = true;
                if(self.inputCurrenciesSelect != 0) self.submitButton.currenciesValid = true;
                if(self.inputCompaniesSelect != 0) self.submitButton.companiesValid = true;
            }else{
                //Control de desactivar banderas
                if(self.inputStatusSelect == 0) self.submitButton.statusValid = false;
                if(self.inputCurrenciesSelect == 0) self.submitButton.currenciesValid = false;
                if(self.inputCompaniesSelect == 0) self.submitButton.companiesValid = false;
            }
        }
    }]
    ...
    .then(request => {
        ...
        for (let cursor = 0; cursor < request.data.departments.length; cursor++) {
            if(cursor != 0){
