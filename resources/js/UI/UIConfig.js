...
export class CrudUi {
    ...
    static limitPagData(self, tableTarget, lengthPage) {
        axios
            .post("/limit-pag", { table: tableTarget, lengthPage: lengthPage })
            .then((request) => {
                ...
                setTimeout(() => {
                    self.maxLengthPagination = Math.ceil(request.data);
                }, AXIOSINTERVAL);
            })
            .catch((error) => {
                console.error(error);
