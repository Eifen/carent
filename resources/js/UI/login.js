...
const loginApp = createApp({
    ...
    mounted() {
        this.controlRecoveryModal = new bootstrap.Modal(
            document.getElementById("modal-forgot-password"),
            { keyboard: false, backdrop: "static" }
        );
    },
    methods: {
        showModal() {
            this.controlRecoveryModal.show();
        },
        changeInput() {
        ...
        iniciarSesion($EncryptKey, $EncryptIv) {
            ...
            try {
                switch (true) {
                    case this.codigoUsuario.value == "" &&
                        this.passwordUsuario.value != "": //Codigo
                        //Cambiamos el estado de los campos
                        this.codigoUsuario.IsEmpty = true;
                        this.passwordUsuario.IsEmpty = false;
                        throw "NoCodigo";
                    case this.codigoUsuario.value != "" &&
                        this.passwordUsuario.value == "": //Password Vacia
                        this.passwordUsuario.IsEmpty = true;
                        this.codigoUsuario.IsEmpty = false;
                        throw "NoPassword";
                    case this.passwordUsuario.value == "" &&
                        this.codigoUsuario.value == "": //Ambos Vacios
                        this.codigoUsuario.IsEmpty = true;
                        this.passwordUsuario.IsEmpty = true;
                        throw "EmptyData";
                    default:
        ...
        login($Password, $User, $KeyEncriptada, $IvEncriptada) {
            ...
            axios
                .post("/login", ParamsLogin)
                .then((request) => {
                    if (!request.data.response && request.status === 200)
        ...
        /**
         * Metodo que envia un correo de recuperacion de contra
         * @param {*} $codeUser Codigo del usuario
         */
        toRecovery($codeUser) {
            this.isClick = true;
            //Enviamos los parametros a la ruta del recovery
            CrudUi.controlCrud(
                { post: "/recovery", redirect: "/", self: this },
                { userCode: $codeUser }
            );
        },
    },
    ...
    watch: {
        ...
        codigoUsuario: {
            ...
            handler(estadoCodigo) {
                ...
                estadoCodigo.IsEmpty === true
                    ? (this.ErrorStyle.HasError.codigo = true)
                    : (this.ErrorStyle.HasError.codigo = false);
            },
        },
        //Password
        passwordUsuario: {
            ...
            handler(estadoPassword) {
                estadoPassword.IsEmpty === true
                    ? (this.ErrorStyle.HasError.password = true)
                    : (this.ErrorStyle.HasError.password = false);
            },
        },
        showPassword(newValue) {
            newValue === true
                ? ((this.controlEye = "fa-solid fa-eye-slash"),
                  (this.TypeInputPassword = "text"))
                : ((this.controlEye = "fa-solid fa-eye"),
